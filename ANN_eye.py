...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = self.frac_tanh(self.fc1(x))
        x = self.frac_tanh(self.fc2(x))
        x = self.frac_tanh(self.fc3(x))
        x = self.frac_tanh(self.fc4(x))
...
'''
# Plot the preprocessed EEG signal
