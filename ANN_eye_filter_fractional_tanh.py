...
class FractionalTanh(nn.Module):
    def __init__(self, fraction=0.5):
        super(FractionalTanh, self).__init__()
        self.fraction = fraction

    def forward(self, x):
        return self.fraction * torch.tanh(x)
...
class Net(nn.Module):
    def __init__(self, fraction=0.5):
        ...
        self.frac_tanh = FractionalTanh(fraction)

    def forward(self, x):
        x = self.dp1(self.bn1(self.frac_tanh(self.fc1(x))))
        x = self.dp2(self.bn2(self.frac_tanh(self.fc2(x))))
        x = self.dp3(self.bn3(self.frac_tanh(self.fc3(x))))
        x = self.dp4(self.bn4(self.frac_tanh(self.fc4(x))))
