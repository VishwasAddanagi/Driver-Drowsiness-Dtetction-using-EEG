...
class FractionalReLU(nn.Module):
    def __init__(self, alpha=0.7):
        super(FractionalReLU, self).__init__()
        self.alpha = alpha
...
class Net(nn.Module):
    def __init__(self,alpha=0.7):
        ...
        self.frac_relu = FractionalReLU(alpha)

    def forward(self, x):
        x = self.frac_relu(self.fc1(x))
        x = self.frac_relu(self.fc2(x))
        x = self.frac_relu(self.fc3(x))
        x = self.frac_relu(self.fc4(x))
